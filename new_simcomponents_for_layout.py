...
class Machine(object):
    def __init__(self, env, name, process_name, parts, resource, process_time, priority, waiting, monitor, MTTF, MTTR,
    ...
    def work(self):
        while True:
            ...
            if self.workforce is True:
                ...
                self.monitor.record(self.env.now, self.process_name, self.name, part_id=part.name, event="workforce_request", resource=resource_item)
            ...
            while proc_time:
                ...
                try:
                    self.broken = False
                    ## working start
                    self.monitor.record(self.env.now, self.process_name, self.name, part_id=part.name, event="work_start")
                    self.working_start = self.env.now
                    yield self.env.timeout(proc_time)

                    ## working finish
                    self.monitor.record(self.env.now, self.process_name, self.name, part_id=part.name, event="work_finish")
